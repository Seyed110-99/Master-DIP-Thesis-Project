...
train_data = ImageDaraset('data/train', transform=transforms)
test_data = ImageDaraset('data/test', transform=transforms)
ds_data_train = DataLoader(train_data, batch_size=16, shuffle=True)
ds_data_val = DataLoader(test_data, batch_size=16, shuffle=False)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for real_img in ds_data_val:
            n_batches += 1
            real_img = real_img.to(device)
            noise = torch.randn_like(real_img) * rand_sigma_test
            noise_img = real_img + noise
            D_real = model(real_img).mean()
    ...
    torch.save(model.state_dict(), 'checkpoints/pre_model_img.pth')
