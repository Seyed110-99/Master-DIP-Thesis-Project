...
def ellipses_DIP_dl(walnut_GT, img_num ,lambs, noise_level = "none", model_type = "ellipses", input_type = "z", critic_noise = "low"):
    ...
    #     plt.axis('off')
        os.makedirs(f"results_img_more/DIP_dl/{model_type}/{noise_level}", exist_ok=True)
    ...
    #     f"PSNR={best_psnr:.2f} dB    SSIM={best_ssim:.4f}",
    #     pad=2,
    #     y=1.02,
    #     va="bottom"
    # )
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img_more/DIP_dl/{model_type}/{noise_level}/best_img_{model_type}_{input_type}_img{img_num+1}_TV.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img_more/DIP_dl/{model_type}/{noise_level}/worst_img_{model_type}_{input_type}_img{img_num+1}_TV.png", dpi=200, bbox_inches='tight')
    plt.close()

    out_dir = f"results_img_more/DIP_dl/{model_type}/{noise_level}/{input_type}"
    ...
    for i in range(9):
        ...
        for model_type in models:
            for noise_level in noise_levels:
                ...
                plt.legend()
                out_dir = f"results_img_more/DIP_dl/{model_type}/{noise_level}"
                os.makedirs(out_dir, exist_ok=True)
