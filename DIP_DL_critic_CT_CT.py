...
def ellipses_DIP_dl(lambs, noise_level = "none", model_type = "ellipses", input_type = "z", critic_noise = "low"):
    ...
    walnut_GT = torch.load("results/ct.pt", map_location=device)
    walnut_GT = walnut_GT.to(device)
    max_walnut = torch.max(walnut_GT).item()

    if noise_level == "none":
        walnut_data = torch.load("results/ct_no_noise.pt", map_location=device)
    elif noise_level == "low":
        walnut_data = torch.load("results/ct_low_noise.pt", map_location=device)
    elif noise_level == "high":
        walnut_data = torch.load("results/ct_high_noise.pt", map_location=device)
    elif noise_level == "very_high":
        walnut_data = torch.load("results/ct_very_high_noise.pt", map_location=device)
    else:
    ...
    if input_type == "z":
        ...
        if critic_noise == "low":
            ...
            critic.load_state_dict(torch.load("checkpoints/pre_model_reg_FBP_high_CT.pth", map_location=device))
        ...
        if critic_noise == "low":
            ...
            critic.load_state_dict(torch.load("checkpoints/pre_model_reg_BP_high_CT.pth", map_location=device))
        else:
            raise ValueError(f"Unknown critic noise level {critic_noise}")
    ...
    for lamb in lambs:
        ...
        # plt.axis('off')
        os.makedirs(f"results_new/DIP_dl_critic_CT/{model_type}/{noise_level}", exist_ok=True)
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_new/DIP_dl_critic_CT/{model_type}/{noise_level}/rec_epoch_{input_type}_{best_lamb:.1e}_best_critic.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_new/DIP_dl_critic_CT/{model_type}/{noise_level}/rec_epoch_{input_type}_{worst_lamb:.1e}_worst_critic.png", dpi=200, bbox_inches='tight')
    plt.close()

    out_dir = f"results_new/DIP_dl_critic_CT/{model_type}/{noise_level}/{input_type}"
    ...
    for critic_noise in critic_noises:
        for model_type in models:
            for noise_level in noise_levels:
                ...
                plt.legend()
                out_dir = f"results_new/DIP_dl_critic_CT/{model_type}/{noise_level}"
                os.makedirs(out_dir, exist_ok=True)
