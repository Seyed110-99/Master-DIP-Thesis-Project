...
def ellipses_DIP_dl(lambs, noise_level = "none", model_type = "ellipses", input_type = "z"):
    ...
    plt.imshow(walnut_GT.squeeze().detach().cpu().numpy(), cmap='gray')
    plt.axis('off')
    plt.savefig(f"results_img/actual_img.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    #     plt.axis('off')
        os.makedirs(f"results_img/DIP_dl/{model_type}/{noise_level}", exist_ok=True)
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img/DIP_dl/{model_type}/{noise_level}/rec_epoch_{input_type}_{best_lamb:.1e}_best_TV_{model_type}.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img/DIP_dl/{model_type}/{noise_level}/rec_epoch_{input_type}_{worst_lamb:.1e}_worst_TV_{model_type}.png", dpi=200, bbox_inches='tight')
    plt.close()

    out_dir = f"results_img/DIP_dl/{model_type}/{noise_level}/{input_type}"
...
if __name__ == "__main__":
    ...
    for model_type in models:
        for noise_level in noise_levels:
            ...
            plt.legend()
            out_dir = f"results_img/DIP_dl/{model_type}/{noise_level}"
            os.makedirs(out_dir, exist_ok=True)
