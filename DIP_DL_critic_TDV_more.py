...
def ellipses_DIP_dl(walnut_GT, img_num ,lambs, noise_level = "none", model_type = "ellipses", input_type = "z", critic_noise = "low"):
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img_more/DIP_dl_tdv/{model_type}/{noise_level}/best_img_{model_type}_{input_type}_img{img_num+1}_TDV.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results_img_more/DIP_dl_tdv/{model_type}/{noise_level}/worst_img_{model_type}_{input_type}_img{img_num+1}_TDV.png", dpi=200, bbox_inches='tight')
    plt.close()
        ...
        for critic_noise in critic_noises:
            for model_type in models:
                for noise_level in noise_levels:
                    ...
                    os.makedirs(out_dir, exist_ok=True)
                    plt.savefig(f"{out_dir}/psnr_compare_{model_type}_{noise_level}_img{i+1}_TDV.png", dpi=200)
                    plt.close()
                    ...
                    plt.legend()
                    plt.savefig(f"{out_dir}/ssim_compare_{model_type}_{noise_level}_img{i+1}_TDV.png", dpi=200)
                    plt.close()
