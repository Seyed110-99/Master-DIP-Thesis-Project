...
def ellipses_DIP_dl(lambs, noise_level = "none", model_type = "ellipses", input_type = "z", critic_noise = "low"):
    ...
    for lamb in lambs:
        ...
        # plt.axis('off')
        os.makedirs(f"results/DIP_dl_critic_CT/{model_type}/{noise_level}", exist_ok=True)
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results/DIP_dl_critic_CT/{model_type}/{noise_level}/rec_epoch_{input_type}_{best_lamb:.1e}_best_critic.png", dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.subplots_adjust(top=0.85)
    plt.axis('off')
    plt.savefig(f"results/DIP_dl_critic_CT/{model_type}/{noise_level}/rec_epoch_{input_type}_{worst_lamb:.1e}_worst_critic.png", dpi=200, bbox_inches='tight')
    plt.close()

    out_dir = f"results/DIP_dl_critic_CT/{model_type}/{noise_level}/{input_type}"
    ...
    for critic_noise in critic_noises:
        for model_type in models:
            for noise_level in noise_levels:
                ...
                plt.legend()
                out_dir = f"results/DIP_dl_critic_CT/{model_type}/{noise_level}"
                os.makedirs(out_dir, exist_ok=True)
