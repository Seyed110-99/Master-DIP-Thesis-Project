...
def classic_TV_solver(lambs, image_path, noise_level = "none"):
    ...
    ct_data_noisy = torch.load(image_path, map_location=device)
    ct_data = ct_data_noisy.to(device)

    ct_GT = torch.load("results/ct.pt", map_location=device)
    ...
    A_norm = physics.compute_norm(physics.A_dagger(ct_data)).item()
    ...
    for lamb in lambs:
        ...
        plt.axis('off')
        os.makedirs(f"results_new/classic/tv_sigma_{noise_level}", exist_ok=True)
        plt.savefig(f"results_new/classic/tv_sigma_{noise_level}/rec_lambda_{lamb:.0e}_step_size{stepsize:.0e}.png", dpi=200, bbox_inches='tight')
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"results_new/classic/tv_sigma_{noise_level}/psnr_trajectories_{best_lamb}_{best_stepsize}_{noise_level}.png", dpi=200)
    plt.close()
    ...
    # plt.legend(bbox_to_anchor=(1.02, 1), loc='upper left', fontsize='small')
    plt.tight_layout()
    plt.savefig(f"results_new/classic/tv_sigma_{noise_level}/ssim_trajectories_{best_lamb}_{best_stepsize}_{noise_level}.png", dpi=200)
    plt.close()
    ...
    out_dir = f"results_new/classic/tv_sigma_{noise_level}"
...
if __name__ == "__main__":
    ...
    image_path_no_noise = "results/ct_no_noise.pt"
    ...
    image_path_low_noise = "results/ct_low_noise.pt"
    ...
    image_path_high_noise = "results/ct_high_noise.pt"
