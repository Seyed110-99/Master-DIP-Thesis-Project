...
#     arr = torch.stack(list(buffer), axis = 0)
#     mean = torch.mean(arr, axis=0)
#     diff = arr - mean
#     variance = torch.mean(diff.pow(2))
...
# W = 100 # window size for variance calculation
# P = 200 # patience for early stopping
...
#     if len(buffer) == W:
#         variance = compute_variance(buffer)
#         print(f"Epoch [{epoch}/{epochs}], Variance: {variance:.6f}, Loss: {loss.item():.4f}")
#         if variance < var_min:
#             var_min = variance
...
#             print("Early stopping triggered due to low variance.")
#             break
#     else:
#         print(f"Epoch [{epoch}/{epochs}], Loss: {loss.item():.4f}")
    
#     if epoch % 200 == 0:
#         # Convert output to CPU for inspection 
#         output_image_np = output.squeeze().detach().cpu().numpy() 
#         output_image_np = output_image_np.transpose(1,2,0) # (C, H, W) to (H, W, C)

#         noisy_image_np = noisy_tensor.squeeze().detach().cpu().numpy()
#         noisy_image_np = noisy_image_np.transpose(1,2,0) # (C, H, W) to (H, W, C)
...
# plt.grid(True)
# plt.savefig(f'outputs/psnr_plot_{noise_sigma}_ES.png')